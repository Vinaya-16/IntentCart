                        ...
                        <main className="p-3.5 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 max-w-7xl mx-auto">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200/80 pb-4 sm:pb-5">
                                <div className="flex items-center gap-2.5 sm:gap-3">
                                    ...
                                    <div className="min-w-0">
                                        <h1 className="text-lg sm:text-2xl font-bold text-slate-900 tracking-tight truncate">Returns Management</h1>
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                                {stats.map((stat, index) => {
                                    ...
                                    return (
                                        <div key={index} className="bg-white p-3.5 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-200/80 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 hover:shadow-xs transition-shadow">
                                            <div>
                                            ...
                                            <div className={`p-2.5 sm:p-3 rounded-lg sm:rounded-xl ring-1 ${colorMap[stat.color].ring} ${colorMap[stat.color].bg} ${colorMap[stat.color].text} shrink-0 self-end sm:self-center`}>
                                ...
                                <div className="block md:hidden divide-y divide-slate-100">
                                    {filteredReturns.length > 0 ? (
                                        filteredReturns.map((ret) => {
                                            ...
                                            return (
                                                <div key={ret.id} className="p-4 space-y-3 bg-white hover:bg-slate-50/50 transition-colors">
                                                    <div className="flex items-start justify-between gap-2">
                                                        <div>
                                                            <span className="font-semibold text-sm text-slate-900">{ret.id}</span>
                                                            <span className="text-xs text-slate-400 ml-2">({ret.orderId})</span>
                                                            <p className="text-xs text-slate-400 mt-0.5">{formatDate(ret.date)}</p>
                                                        </div>
                                                    ...
                                                    <div className="flex items-center gap-2.5 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            <p className="text-xs font-semibold text-slate-800 truncate">{ret.product}</p>
                                                            <p className="text-[11px] text-slate-500 truncate">Reason: {ret.reason}</p>
                                                        </div>
                                                    </div>

                                                    <div className="flex items-center justify-between text-xs pt-1">
                                                        <div>
                                                            <p className="font-medium text-slate-800">{ret.customer}</p>
                                                            <p className="text-[11px] text-slate-400">{ret.refundMethod}</p>
                                                        </div>
                                                        <div className="text-right flex items-center gap-3">
                                                            <div>
                                                                <p className="font-bold text-sm text-slate-900">{formatCurrency(ret.amount)}</p>
                                                            </div>
                                                            <button
                    ...
                    <div className="bg-white rounded-2xl sm:rounded-3xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
                        ...
                        <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto flex-1">
                            <div className="p-3 sm:p-3.5 bg-slate-50 rounded-2xl border border-slate-200/60 flex items-center gap-3">
                                <div className="p-2.5 bg-white rounded-xl border border-slate-200/80 text-slate-700 shadow-2xs shrink-0">
                                    <Package className="w-5 h-5" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Product Item</p>
                                    <p className="text-xs sm:text-sm font-semibold text-slate-900 truncate">{selectedReturn.product}</p>
                                </div>
                                <div className="text-right shrink-0">
                                    <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Refund</p>
                                    <p className="text-xs sm:text-sm font-bold text-slate-900">{formatCurrency(selectedReturn.amount)}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:gap-4 text-xs">
                                <div className="p-3 rounded-xl bg-slate-50/80 border border-slate-100">
                                    ...
                                    <p className="font-medium text-slate-800 text-xs sm:text-sm mt-0.5 truncate">{selectedReturn.customer}</p>
                                </div>
                                <div className="p-3 rounded-xl bg-slate-50/80 border border-slate-100">
                                    <p className="font-semibold text-slate-400 uppercase tracking-wider text-[10px]">Payment Method</p>
                                    <p className="font-medium text-slate-800 text-xs sm:text-sm mt-0.5 truncate">{selectedReturn.refundMethod}</p>
                                </div>
                            </div>
                        ...
                        <div className="p-4 sm:p-5 border-t border-slate-100 bg-slate-50/30 flex flex-col sm:flex-row justify-end gap-2.5 shrink-0">
                            <button
                                ...
                                className="w-full sm:w-auto px-4 py-2.5 border border-rose-200 text-rose-600 hover:bg-rose-50 rounded-xl text-xs font-semibold transition-all"
                            >
                                Reject Return
                            </button>
                            <button
                                onClick={() => handleAction('Approve & Schedule Pickup')}
                                className="w-full sm:w-auto px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold shadow-2xs transition-all"
                            >
